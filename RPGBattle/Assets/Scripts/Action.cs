...
public class ActionDoT : Action
{
    ...
    public ActionDoT(Agent source) 
        : base("DoT", source, Constants.DEBUFF_DURATION, Constants.DOT_INTERVAL)
    {
...
//Buff class. If no stat is provided, will randomly select one buff to apply.
public class ActionBuff : Action
{
    private string stat;

    public ActionBuff(Agent source, string stat = "none") 
    ...
    public override void Execute(Agent target)
    {
        ...
        if (stat == "none")
        {
            ...
            switch (randomIndex)
            {
                case 0: stat = "attack"; name = $"{stat}"; break;
                case 1: stat = "defense"; name = $"{stat}"; break;
                case 2: stat = "speed"; name = $"{stat}"; break;
            }
        }
        switch (stat)
        {
...
public class ActionHoT : Action
{
    ...
    public ActionHoT(Agent source) 
        : base("HoT", source, Constants.BUFF_DURATION, Constants.HOT_INTERVAL)
    {
